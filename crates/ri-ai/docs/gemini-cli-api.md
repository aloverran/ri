...
The two endpoints serve **completely disjoint model sets**:
...
| Models | 2.5 Pro, 2.5 Flash, 2.0 Flash | 3.1 Pro, 3 Flash |
| Thinking control | `thinkingBudget` (token count) | `thinkingLevel` (named level) |
...
Probed 2026-02-23. Only exact model IDs work; `-preview`, `-latest`, `-exp` suffixes
all return 404.
...
### 403 on model access
The model exists but requires different auth tier. 3.x models need Antigravity
credentials. This is an endpoint mismatch, not a token issue.
