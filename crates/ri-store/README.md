...
**pool** — `Pool`: a `HashMap<String, Message>` with insertion-order tracking. Core operations: `put`, `get`, `iter`, `resolve` (bulk ID lookup), `derived_from` (find messages produced from a given input). The pool is the in-memory universe of all messages.
